  ...
  <div :class="tdClass">
    ...
    <table>
      <tr>
        <td :colspan="colSpan">
          <table>
              <tr class="leve" :class="levelClass" @click="getRowFunc(model)">
                <td class="td1">
                  <div class="td-title" @dblclick="handlerExpand(model)">
                    ...
                    <a class="ellipsis">
                      <i v-if="model.children.length > 0" class="t-icon m-dep1"></i>
                      <i v-else class="t-icon m-dep"></i>
                      <span :title="model.name">{{model.name}}</span>
                    </a>
                  </div>
                </td>
                <td class="td2">
                   <span v-if="model.level == '1'">一级指标</span>
					<span v-if="model.level == '2'">二级指标</span>
					<span v-if="model.level == '3'">三级指标</span>
                </td>
                <!-- <td class="td3">{{model.CreateTime|formatDate}}</td> -->
                <td class="td4">
                  <span :title="model.Experience" class="ellipsis">{{model.score}}</span>
                </td>
                <td class="td6">
                  <a class="reset" href="javascript:;" @click="subordinateFunc(model)">添加下级指标</a>
				  <i class="line"></i>
				  <a class="reset" href="javascript:;" v-hasPermi="['btn:libForm:config']" @click="configFunc(model)">配置</a>
				  <i class="line"></i>
...
<script>
  export default {
	...
	methods: {
		...
		getRowFunc(m){
			this.$emit('getRowFunc',m)
		},
		configFunc(m){
			this.$emit('configFunc',m)
		},
		subordinateFunc(m){
			this.$emit('subordinateFunc',m)
		}
	},
	filters: {
