...
import { saveMyh, getMyhResult } from '@/api/review'
...
export default {
  data() {
    return {
      ...
      gradeTotalMyhId: this.$route.query.gradeTotalMyhId,
      isFixed: false,
  ...
  methods: {
    ...
    getData () {
      this.loading = true
      getMyhResult({ gradeTotalMyhId: this.gradeTotalMyhId }).then(res => {
        this.loading = false
    ...
    handleNumCon(row) {
      ...
      subArr.gradeTotalMyhId = this.gradeTotalMyhId
      this.submitData = subArr
      this.disabled = false
    },
    subScore() {
      this.btnLoading = true
      saveMyh(this.submitData).then(res => {
        if (res.code == 1) {
          this.disabled = true
          this.getData()
