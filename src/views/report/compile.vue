        ...
        <el-col :span="24">
          <div style="height:calc(100vh - 100px);padding:4px;box-shadow: 0 0 10px 2px rgba(30,13,4,.15);">
            <el-scrollbar style="height: 100%;">
              <!-- 滚动条 -->
              <el-table :data="statisticDatas" class="tableStyle" v-loading="leftTb" style="width:100% !important;" size="medium" :row-style="selectedHighlight" :row-class-name="rowClassNameHandler" @row-click="selectRow">
                <el-table-column label="指标名称" align="left" show-overflow-tooltip>
                  <template slot-scope="scope">
                    <span :style="{marginLeft: scope.row.level * 23 + 'px'}">&ensp;</span>
                    <i v-if="scope.row.showChildren" :class="{'categoryStyle el-icon-folder-opened ':scope.row.showChildren, 'categoryStyle el-icon-document-remove':!scope.row.hasChildren}" @click="onExpand(scope.row)"
                      :style="{cursor:scope.row.hasChildren ? 'pointer' : 'normal'}"></i>
                    <i v-else :class="{'categoryStyle el-icon-folder':scope.row.hasChildren, 'categoryStyle el-icon-document-remove':!scope.row.hasChildren}" @click="onExpand(scope.row)"
                      :style="{cursor:scope.row.hasChildren ? 'pointer' : 'normal'}"></i>
      ...
      <el-row :gutter="20" class="mt15" v-loading="loading" v-if="!keyPointShow">
        <div class="rightbox" v-if="noDataVisible">
          <el-scrollbar style="height: 100%;">
            ...
            <dynamic-component :name="rowCpt" :aid="currentRow" :compileTime="compileTime" :compileId="compileId" :createUserId="createUserId" :awardId="awardId">
            </dynamic-component>
          </el-scrollbar><!-- /滚动条 -->
        </div>
        <div class="rightbox" v-else>
          <el-scrollbar style="height: 100%;">
            ...
            <dynamic-component :name="rowCpt" :aid="currentRow" :compileId="compileId" :compileTime="compileTime" :createUserId="createUserId" :awardId="awardId">
            </dynamic-component>
          </el-scrollbar><!-- /滚动条 -->
        </div>
      </el-row>
...
.statistics {
    ...
    .awardInfo {
        background: #fff;
        box-shadow: rgba(30, 13, 4, 0.15) 0 0 10px 2px;
        padding: 20px;
        overflow: hidden;
        .awardName {
            margin-bottom: 15px;
